...
async def create_airline_user(res: Response, phone_number: str =  Body(embed=True), username: str = Body(embed=True), disp_name: str = Body(embed=True), email: EmailStr = Body(embed=True), password: str = Body(embed=True), airline_id: str = Body(embed=True), x_auth=Header(alias="X-Auth", default=None)) -> JsonResponse:
    """
    ...
    await log_man.add_log(
        func_id,
        "DEBUG",
        f"received create airline user request: username={auth_service_response.data['token_claims']['username']} organization={auth_service_response.data['token_claims']['organization']} airline_id = {airline_id}",
    )
    organization = auth_service_response.data["token_claims"]["organization"]

    db_service_response = await users_database_api.create_airline_user_db(username=username, disp_name=disp_name,email=email,phone_number=phone_number,password=password,airline_id=airline_id,organization=organization)
