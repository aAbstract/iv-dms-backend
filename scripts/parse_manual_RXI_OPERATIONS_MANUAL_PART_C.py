...
def create_parts_metadata_file():
    metadata = []
    manual_parts = glob(r"data/RXI_OPERATIONS_MANUAL_PART_C/*.pdf")
    ...
    with open("data/RXI_OPERATIONS_MANUAL_PART_C/RXI_OPERATIONS_MANUAL_PART_C_metadata.json", "w") as f:
...
def create_manual_toc_tree():
        ...
        for line in lines:
            if toc_epattern in line:
                ...
                #     page_number = -1
                    ...
                    if(title.split()[0] == section_code[:section_code.find(".")]):
                        toc_info.append((section_code + " "+ section_name, page_number))

        return toc_info

    f = open("data/RXI_OPERATIONS_MANUAL_PART_C/RXI_OPERATIONS_MANUAL_PART_C_metadata.json", "r")
    json_str = f.read()
    f.close()
    ...
    for mde in json_obj:
        toc_txt = ""
        pdf_reader = PdfReader("data/RXI OMC _08.02.24.pdf")
    ...
    f = open("data/RXI_OPERATIONS_MANUAL_PART_C/RXI_OPERATIONS_MANUAL_PART_C_second_metadata.json", "w")
...
def rearrange_manual_content_tree() -> list[object]:
    ...
    path_to_metadata = "data/RXI_OPERATIONS_MANUAL_PART_C/RXI_OPERATIONS_MANUAL_PART_C_second_metadata.json"
    all_chapters = []
    all_sub1_sections = []
    all_sub2_section = []
    all_sub3_section = []
    ...
    with open("data/RXI_OPERATIONS_MANUAL_PART_C/RXI_OPERATIONS_MANUAL_PART_C_second_metadata_tree.json", "w") as json_file:
...
print("Complete Parse of RXI_OPERATIONS_MANUAL_PART_C")